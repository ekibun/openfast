...
! This code was generated by "Write_ChckOutLst.m" at 29-Oct-2025 13:26:57.
  ...
  INTEGER(IntKi), PARAMETER      :: MaxStCControlOuts = 10 ! maximum number of StC control-channel outputs

  INTEGER(IntKi), PARAMETER      :: StC_Kin_Num = 18
  INTEGER(IntKi), PARAMETER      :: StC_Kin_PX  =  1
  INTEGER(IntKi), PARAMETER      :: StC_Kin_PY  =  2
  INTEGER(IntKi), PARAMETER      :: StC_Kin_PZ  =  3
  INTEGER(IntKi), PARAMETER      :: StC_Kin_VX  =  4
  INTEGER(IntKi), PARAMETER      :: StC_Kin_VY  =  5
  INTEGER(IntKi), PARAMETER      :: StC_Kin_VZ  =  6
  INTEGER(IntKi), PARAMETER      :: StC_Kin_AX  =  7
  INTEGER(IntKi), PARAMETER      :: StC_Kin_AY  =  8
  INTEGER(IntKi), PARAMETER      :: StC_Kin_AZ  =  9
  INTEGER(IntKi), PARAMETER      :: StC_Kin_RX  = 10
  INTEGER(IntKi), PARAMETER      :: StC_Kin_RY  = 11
  INTEGER(IntKi), PARAMETER      :: StC_Kin_RZ  = 12
  INTEGER(IntKi), PARAMETER      :: StC_Kin_WX  = 13
  INTEGER(IntKi), PARAMETER      :: StC_Kin_WY  = 14
  INTEGER(IntKi), PARAMETER      :: StC_Kin_WZ  = 15
  INTEGER(IntKi), PARAMETER      :: StC_Kin_ALX = 16
  INTEGER(IntKi), PARAMETER      :: StC_Kin_ALY = 17
  INTEGER(IntKi), PARAMETER      :: StC_Kin_ALZ = 18
...
!..................................................................................................................................
   ...
   INTEGER(IntKi)               :: StCKinIndx                                      ! Index for dynamic StC kinematic output

   LOGICAL                      :: CheckOutListAgain                               ! Flag used to determine if output parameter starting with "M" is valid (or the negative of another parameter)
   LOGICAL                      :: InvalidOutput(0:MaxOutPts)                      ! This array determines if the output channel is valid for this configuration
   LOGICAL                      :: StCKinFound                                     ! This dynamic StC kinematic channel exists
   LOGICAL                      :: StCKinInvalid                                   ! This dynamic StC kinematic channel is invalid for this configuration
   CHARACTER(ChanLen)           :: OutListTmp                                      ! A string to temporarily hold OutList(I)
   CHARACTER(ChanLen)           :: StCKinUnits                                     ! Units for dynamic StC kinematic output
   ...
   DO I = 1,p%NumOuts
      ...
      StCKinInvalid = .FALSE.
      IF ( Indx < 1 ) THEN
         CALL Parse_StC_Kin_OutParam( OutListTmp(1:OutStrLenM1), p, StCKinFound, StCKinInvalid, StCKinIndx, StCKinUnits )
      END IF
      ...
      IF ( Indx > 0 ) THEN ! we found the channel name
         ...
         END IF
      ELSE IF ( StCKinFound ) THEN ! dynamic StC section kinematic channel
         p%OutParam(I)%Indx = StCKinIndx
         IF ( StCKinInvalid ) THEN
            p%OutParam(I)%Units = "INVALID"
            p%OutParam(I)%SignM = 0
         ELSE
            p%OutParam(I)%Units = StCKinUnits
         END IF
...
END FUNCTION StC_Kin_Index_Ch
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE Set_StC_Kin_Ch_Outs( p_SrvD, u_BStC, u_NStC, u_TStC, u_SStC, AllOuts )

   TYPE(SrvD_ParameterType),                 INTENT(IN   ) :: p_SrvD
   TYPE(StC_InputType), ALLOCATABLE,         INTENT(IN   ) :: u_BStC(:,:)
   TYPE(StC_InputType), ALLOCATABLE,         INTENT(IN   ) :: u_NStC(:,:)
   TYPE(StC_InputType), ALLOCATABLE,         INTENT(IN   ) :: u_TStC(:,:)
   TYPE(StC_InputType), ALLOCATABLE,         INTENT(IN   ) :: u_SStC(:,:)
   REAL(ReKi),                               INTENT(INOUT) :: AllOuts(0:MaxOutPts)

   INTEGER(IntKi)                                          :: i, j, chan
   INTEGER(IntKi)                                          :: FirstIndx, LastIndx
   INTEGER(IntKi)                                          :: NumPerChan(MaxStCControlOuts)
   ...
   DO chan=1,MaxStCControlOuts
      IF ( NumPerChan(chan) > 1_IntKi ) THEN
         FirstIndx = StC_Kin_Index_Ch(chan,StC_Kin_PX)
         LastIndx  = StC_Kin_Index_Ch(chan,StC_Kin_ALZ)
         AllOuts(FirstIndx:LastIndx) = AllOuts(FirstIndx:LastIndx) / REAL(NumPerChan(chan),ReKi)
      END IF
   END DO
...
SUBROUTINE Accum_StC_Kin_Outs( Chan, u, j, AllOuts, NumPerChan )
   ...
   INTEGER(IntKi),      INTENT(IN   ) :: Chan
   TYPE(StC_InputType), INTENT(IN   ) :: u
   INTEGER(IntKi),      INTENT(IN   ) :: j
   REAL(ReKi),          INTENT(INOUT) :: AllOuts(0:MaxOutPts)
   INTEGER(IntKi),      INTENT(INOUT) :: NumPerChan(MaxStCControlOuts)

   REAL(ReKi)                         :: Kin(StC_Kin_Num)
   REAL(ReKi)                         :: RotDisp(3)
   INTEGER(IntKi)                     :: FirstIndx, LastIndx

   IF ( Chan < 1_IntKi .OR. Chan > MaxStCControlOuts ) RETURN
   IF ( .NOT. ALLOCATED(u%Mesh) ) RETURN
   IF ( j < LBOUND(u%Mesh,1) .OR. j > UBOUND(u%Mesh,1) ) RETURN
   FirstIndx = StC_Kin_Index_Ch(Chan,1)
   LastIndx  = FirstIndx + StC_Kin_Num - 1
   IF ( FirstIndx < 1_IntKi .OR. LastIndx > MaxOutPts ) RETURN

   RotDisp = REAL( EulerExtractZYX( REAL(u%Mesh(j)%Orientation(:,:,1), R8Ki) ), ReKi )
   Kin(StC_Kin_PX :StC_Kin_PZ ) = u%Mesh(j)%Position(1:3,1) + u%Mesh(j)%TranslationDisp(1:3,1)
   Kin(StC_Kin_VX :StC_Kin_VZ ) = u%Mesh(j)%TranslationVel(1:3,1)
   Kin(StC_Kin_AX :StC_Kin_AZ ) = u%Mesh(j)%TranslationAcc(1:3,1)
   Kin(StC_Kin_RX :StC_Kin_RZ ) = RotDisp(1:3)*R2D
   Kin(StC_Kin_WX :StC_Kin_WZ ) = u%Mesh(j)%RotationVel(1:3,1)*R2D
   Kin(StC_Kin_ALX:StC_Kin_ALZ) = u%Mesh(j)%RotationAcc(1:3,1)*R2D

   AllOuts(FirstIndx:LastIndx) = AllOuts(FirstIndx:LastIndx) + Kin
   NumPerChan(Chan) = NumPerChan(Chan) + 1_IntKi

END SUBROUTINE Accum_StC_Kin_Outs
!----------------------------------------------------------------------------------------------------------------------------------
